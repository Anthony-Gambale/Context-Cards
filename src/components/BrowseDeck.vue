<template>
  ...
  <div class="input-group mb-3">
    ...
    <button class="btn btn-outline-secondary" type="button" id="button-addon2" @click="searchButtonClicked">{{ buttonText
    }}</button>
  </div>
  ...
  <div v-for="(pile, i) in readDeckWrapper()" :key="i">
    ...
    <BrowseCard v-for="(card, j) in pile" :key="j" :card="card" :pile="i" :remove="removeFromDeck" />
  </div>
</template>
